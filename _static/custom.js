...
function add_ticks() {
    const html_code_tick_vert = '<svg role="img" height="16" viewBox="0 0 16 16" version="1.1" width="16" data-view-component="true"><path fill-rule="evenodd" d="M13.78 4.22a.75.75 0 010 1.06l-7.25 7.25a.75.75 0 01-1.06 0L2.22 9.28a.75.75 0 011.06-1.06L6 10.94l6.72-6.72a.75.75 0 011.06 0z" fill="#28a745" stroke="#28a745"></path></svg>';
    ...
    for (let i = 0; i < localStorage.length; i++) {
        ...
        if(key.startsWith("Pythonpad-save-files-")) {
            ...
            if(".passed.json" in d_files) {
                list_passed.push(key.slice(preffix_len).replace(".", "_"));
            }
        } else if (key.startsWith("Pythonpad-save-src-")) {
            const preffix_len = "Pythonpad-save-src-".length;
            if(!list_passed.includes(key.slice(preffix_len).replace(".", "_"))) {
                list_en_cours.push(key.slice(preffix_len).replace(".", "_"));
            }
        }
    }
    ...
    for (let item of elements) {
        ...
        if(is_in_preffix_list(href_attribute, list_passed)) {
            item.innerHTML = item.innerHTML + ' ' + html_code_tick_vert;
        } else if(is_in_preffix_list(href_attribute, list_en_cours)) {
            item.innerHTML = item.innerHTML + ' ' + html_code_tick_orange;
        }
    }
}
